class InstrumentVersion
  ...
  def self.build(params = {})
    ...
    if instrument.current_version_number > version_number
      versions = Rails.cache.fetch("instrument_versions-#{instrument.id}", expires_in: 24.hours) { instrument.versions }
      instrument_version.version = Rails.cache.fetch("instrument_versions-#{instrument.id}-#{version_number}", expires_in: 24.hours) do
  ...
  def questions
    unless @version
      return Rails.cache.fetch("questions-#{@instrument.id}", expires_in: 24.hours) do
        @instrument.questions
      end
    end
    Rails.cache.fetch("questions-#{@instrument.id}-#{@version.id}", expires_in: 24.hours) do
      questions = []
  ...
  def find_question_by(options = {})
    ...
    unless @version
      return Rails.cache.fetch("f_q_b-#{@instrument.id}-#{finder}-#{value}", expires_in: 24.hours) do
        @instrument.questions.send("find_by_#{finder}".to_sym, value)
      end
    end
    question = Rails.cache.fetch("f_q_b_v-#{@instrument.id}-#{finder}-#{value}-#{@version.id}", expires_in: 24.hours) do
  ...
  def versioned(object)
    ...
    Rails.cache.fetch("versioned-#{@instrument.id}-#{object.class.name}-#{object.id}-#{@version.id}", expires_in: 24.hours) do
  ...
  def options_for_question(question)
    ...
    unless @version
      return Rails.cache.fetch("o_f_q-#{@instrument.id}-#{question.question_identifier}", expires_in: 24.hours) do
        question.options
      end
    end
    Rails.cache.fetch("o_f_q-#{@instrument.id}-#{question.question_identifier}-#{@version.id}", expires_in: 24.hours) do
      options = []
