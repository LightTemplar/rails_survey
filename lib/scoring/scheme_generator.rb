    ...
    if row[6].strip == 'Manual'
      ...
      if question_identifiers.length > 1 && !row[2].include?('Roster')
        ...
        question_identifiers.each do |identifier|
          scoring_scheme << ManualScheme.new(identifier.strip, row[2].strip, row[6].strip, row[7], row[10].to_i, subdomain)
        end
      else
        scoring_scheme = ManualScheme.new(row[0].strip, row[2].strip, row[6].strip, row[7], row[10].to_i, subdomain)
      end
    elsif row[6].strip == 'Bank scoring'
      scoring_scheme = BankScheme.new(row[0].strip, row[2].strip, row[6].strip, row[7], row[10].to_i, subdomain)
      ...
      scoring_scheme = MatchingScheme.new(row[0].strip, row[2].strip, row[6].strip, row[7], row[10].to_i, subdomain)
      parse_question_identifiers(scoring_scheme, row)
    elsif row[6].strip == 'Indexed' && row[2].strip != 'Roster'
      scoring_scheme = IndexedScheme.new(row[0].strip, row[2].strip, row[6].strip, row[7], row[10].to_i, subdomain)
      parse_question_identifiers(scoring_scheme, row)
      ...
      scoring_scheme = SearchScheme.new(row[0].strip, row[2].strip, row[6].strip, row[7], row[10].to_i, subdomain)
      white_space_index = row[5].index(' ')
      ...
      scoring_scheme = LookupScheme.new(row[0].strip, row[2].strip, row[6].strip, row[7], row[10].to_i, subdomain)
    elsif row[6].strip == 'Sum'
      scoring_scheme = SumScheme.new(row[0].strip, row[2].strip, row[6].strip, row[7], row[10].to_i, subdomain)
      ...
      scoring_scheme = IntegerScheme.new(row[0].strip, row[2].strip, row[6].strip, row[7], row[10].to_i, subdomain)
      scoring_scheme.key_score_mapping = row[5] unless row[5].blank?
    elsif row[6].strip == 'Group average'
      scoring_scheme = GroupAverageScheme.new(row[9].strip, row[1].to_s, row[2].strip, row[6].strip, row[7], row[10].to_i, subdomain)
      ...
      scoring_scheme = ChildRosterScheme.new(row[0].strip, row[2].strip, row[6].strip, row[7], row[10].to_i, subdomain)
      scoring_scheme.question_text = row[3].strip
      scoring_scheme.key_score_mapping = row[5] unless row[5].blank?
    elsif row[2].strip == 'Roster' && row[6].strip == 'Calculation'
      scoring_scheme = StaffRosterScheme.new(row[0].strip, row[2].strip, row[6].strip, row[7], row[10].to_i, subdomain)
