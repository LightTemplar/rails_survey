...
(1..10).each do |num|
  i = p.instruments.create!(title: "Test Instrument #{num}", language: "en", alignment: "left")
  ActiveRecord::Base.transaction do
    p "Created #{i.title}"
    300.times do |q_n|
      question_type = Settings.question_types.sample
      question = i.questions.create!(text: "Question #{q_n}",
                                     question_identifier: "p_#{p.id}i_#{num}q_#{q_n}",
                                     question_type: question_type, number_in_instrument: "#{q_n+1}")
