...
class SurveyPdf
  ...
  def initialize(survey, language, column_count)
    super()
    @survey = survey
    @language = language
  ...
  def header
    formatted_text [{ text: 'Title: ', styles: [:bold] }, { text: "#{@survey.instrument_title} ", font: 'Courier' },
                    { text: 'Language: ', styles: [:bold] }, { text: "#{@instrument.language_name(@language)} ", font: 'Courier' },
  ...
  def content
    column_box([0, cursor], columns: @column_count, width: bounds.width) do
      @instrument.sections.includes(:translations).each do |section|
        section_text = @language == @instrument.language ? section.title : section.translations.where(language: @language)&.first&.text
        section_text ||= section.title
        format_section_text(section_text)
        section.displays.includes(:display_translations).each do |display|
          display_text = @language == @instrument.language ? display.title :
          display.display_translations.where(language: @language)&.first&.text
  ...
  def write_question(iq, response)
    ...
    if ins
      ins_text = @language == @instrument.language ? ins.text :
      ins.instruction_translations.where(language: @language)&.first&.text
      ins_text ||= ins.text
      text sanitize_text("<i>#{ins_text}</i>"), color: '808080', inline_format: true if ins_text
    end
    q_text = @language == @instrument.language ? iq.text :
    iq.question.translations.where(language: @language)&.first&.text
    q_text ||= iq.text
    text sanitize_text(q_text), inline_format: true
    pop_ins = iq.question.pop_up_instruction
    if pop_ins
      pop_ins_text = @language == @instrument.language ? pop_ins.text :
      pop_ins.instruction_translations.where(language: @language)&.first&.text
    ...
    if at_ins
      at_ins_text = @language == @instrument.language ? at_ins.text :
      at_ins.instruction_translations.where(language: @language)&.first&.text
    ...
    indent(20) do
      ...
      if o_ins
        o_ins_text = @language == @instrument.language ? o_ins.text :
        o_ins.instruction_translations.where(language: @language)&.first&.text
          ...
          unless response&.text.blank?
            CSV.parse(response&.text)[0]&.each_with_index do |res, index|
              if iq.question.other? && res.to_i == iq.question.other_index
                o_text = @language == 'es' ? 'Otra ' : 'Other '
                text "#{o_text}  #{response&.other_response}"
              else
                if iq.question.list_of_boxes_variant?
                  o_text = @language == @instrument.language ? iq.question.options[index].to_s :
                  iq.question.options[index].translations.where(language: @language)&.first&.text
                  o_text ||= iq.question.options[index].text
                  text "• #{full_sanitizer.sanitize(o_text)}:  #{res}" unless res.blank?
                else
                  o_text = @language == @instrument.language ? iq.question.options[res.to_i].to_s :
                  iq.question.options[res.to_i].translations.where(language: @language)&.first&.text
