...
class Metric < ApplicationRecord
  ...
  def crunch_metadata_stats
    identifiers = key_name == 'Center ID' ? instrument.surveys.collect(&:center_id).compact :
        instrument.surveys.collect(&:participant_id).compact
  ...
  def record_stat(attr_name, count, percentage)
    ...
    if stat
      stat.update_attributes(count: count, percent: (percentage || ''))
    else
      Stat.create(key_value: attr_name, metric_id: id, count: count, percent: (percentage || ''))
    end
  end
end
