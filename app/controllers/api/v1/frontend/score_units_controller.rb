        ...
        def create
          ...
          if score_unit.save
            params[:option_scores].each do |opt|
              OptionScore.create(score_unit_id: score_unit.id, option_id: opt['option_id'], value: opt['value'], label: opt['label'])
            end
          ...
          if current_user
            ...
            if params[:option_scores]
              ...
              params[:option_scores].each do |option|
                OptionScore.create(score_unit_id: score_unit.id, option_id: option['option_id'], value: option['value'], label: option['label'])
              end
            else
              score_unit.option_scores.delete_all
            end
