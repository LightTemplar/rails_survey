module SynchAble
  ...
  def paranoid_synch_models(table_name, last_synch_time)
    cache_key_1 = "#{id}#{send(table_name).maximum(:updated_at).try(:to_i)}-#{table_name}"
    association = Rails.cache.fetch(cache_key_1) do
      if table_name == 'questions'
        send(table_name).includes(:instrument)
      elsif table_name == 'options'
        send(table_name).includes(:grid_label, question: [:instrument])
      else
        send(table_name)
      end
    end
